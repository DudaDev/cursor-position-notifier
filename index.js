...
CursorPositionNotifier.prototype.start = function(){
	...
	$('body').on(
		'mouseenter.CursorPositionNotifier mouseleave.CursorPositionNotifier',
	 	...
	 	_.debounce(this._onElementSwitch.bind(this), this.debounceDelay));
};
...
CursorPositionNotifier.prototype._onElementSwitch = function(event){
	...
	if (this.isActive) {
		...
		this._initCurrentTracking(parsed);
	}
};
...
CursorPositionNotifier.prototype._initCurrentTracking = function(info){
	...
	if (info.cursorOn){
		element.on('mousemove.cursorPositionNotifier',_.debounce(this._onCurrentMousemove.bind(this), this.debounceDelay / 2));	
	}
